...
# -------------------------------------------------------
# A) CONFIGURACIÓN DE RUTA DE BASE DE DATOS (EN 'data/')
# -------------------------------------------------------
DB_DIR = "data"
DB_NAME = "app_data.db"
...
def create_db():
    """
    Crea (si no existe) la base de datos local en 'data/app_data.db'
    con dos tablas: 'users' y 'records'.
    """
...
def register_user(username, password):
    """
    Registra un nuevo usuario en 'users'. Retorna True si se registró,
...
def login_user(username, password):
    """
    Verifica si (username, password) existe en la tabla 'users'.
...
def add_record(username, grams):
    """
    Inserta un nuevo récord (username, grams) en la tabla 'records'.
    """
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
...
def get_leaderboard():
    """
    Retorna una lista [(username, total_grams), ...] ordenada desc
    según la suma de 'grams'.
    """
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute("""
...
def get_all_users_stats():
    """
    Retorna [(username, total_grams), ...] con la suma total de gramos
    por usuario, ordenado alfabéticamente.
    """
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute("""
...
# -------------------------------------------------------
# C) CLASE QUE MUESTRA LA IMAGEN init.png (LOGIN / REGISTRO)
# -------------------------------------------------------
class InitWindow:
    def __init__(self, master):
        ...
        # Vincular clics
        self.master.bind("<Button-1>", self.handle_init_click)

    def handle_init_click(self, event):
        x, y = event.x, event.y
...
# -------------------------------------------------------
# D) CLASE PRINCIPAL (SIMULADOR YOLO)
# -------------------------------------------------------
class GarbageCollectorSim:
    def __init__(self, root, username):
        ...
        self.model = YOLO("yolo_models/final_best.pt")
        
        self.image_confidence_threshold = 0.25
        self.video_confidence_threshold = 0.4
    ...
    def handle_menu_click(self, event):
        ...
        # Ejemplo: botón para mostrar estadísticas globales
    ...
    def show_simulator(self, image_path=None):
        ...
        if self.video_mode:
            ...
            self.detect_and_display(cv2.imread(image_path))
        ...
        if self.custom_image_mode:
            ...
            self.root.bind("<Button-1>", self.handle_camera_click)
        else:
    ...
    def handle_custom_click(self, event):
        ...
        self.check_bottle_collection(x, y)

    def handle_camera_click(self, event):
        x, y = event.x, event.y
    ...
    def check_bottle_collection(self, x, y):
    ...
    def detect_and_display(self, frame):
        frame = self.resize_to_coordinates(frame, 104, 166, 450, 425)
        if self.video_mode:
    ...
    def resize_to_coordinates(self, image, x1, y1, x2, y2):
        target_width = x2 - x1
        target_height = y2 - y1
        h, w = image.shape[:2]
...
if __name__ == "__main__":
    # Primero creamos la base de datos en 'data/app_data.db'
    create_db()

    # Abrimos la ventana inicial con la imagen de bienvenida
    root = tk.Tk()
    app = InitWindow(root)
    root.mainloop()
